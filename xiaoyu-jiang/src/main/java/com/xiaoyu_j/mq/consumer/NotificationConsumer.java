...
import com.xiaoyu_j.mapper.NotificationMapper;
...
public class NotificationConsumer {
    ...
    public void handleNotificationPush(NotificationMessage message) {
        try {
            ...
            if (sessionManager.isUserOnline(message.getUserId())) {
    ...
    private void pushToWebSocket(NotificationMessage message, Long notificationId) {
        try {
            ...
            wsMessage.put("created_at", message.getCreatedAt().toString());
            wsMessage.put("timestamp", System.currentTimeMillis());
            
            boolean sent = sessionManager.forwardMessageToUser(message.getUserId(), wsMessage);

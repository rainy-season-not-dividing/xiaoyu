...
import com.xiaoyua.mapper.jFavMapper;
...
@Service
@Slf4j
public class jFavServiceImpl implements jFavService {
    @Autowired
    private jFavMapper jFavMapper;
    ...
    public void deleteFavorite(Long itemId, Long userId, String itemType) {
        ...
        // 更新统计：仅对动态收藏-1
        if ("POST".equalsIgnoreCase(itemType)) {
            jPostStatMapper.decFav(itemId);
        }
    }

    @Override
    public boolean isFaved(Long itemId, Long userId, String itemType) {
        Long count = jFavMapper.selectCount(
                new QueryWrapper<FavoritePO>()
                        .eq("user_id", userId)
                        .eq("item_id", itemId)
                        .eq("item_type", itemType.toUpperCase())
    ...
    private void createFavoriteNotificationViaPushService(Long itemId, Long fromUserId, String itemType) {
        try {
            ...
            // 通过推送服务发送收藏通知
            jPushService.pushFavoriteNotification(toUserId, fromUserId, itemId, itemType);

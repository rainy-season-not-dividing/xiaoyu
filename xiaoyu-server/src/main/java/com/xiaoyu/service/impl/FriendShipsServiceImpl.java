...
import com.xiaoyu.mapper.FriendShipsMapper;
...
@Service
public class FriendShipsServiceImpl extends ServiceImpl<FriendShipsMapper, FriendshipsPO> implements FriendShipsService {
    ...
    public void  sendFriendRequest(SendFriendRequestDTO sendFriendRequestDTO) {
        ...
        friendMessagesService.save(friendMessagesPO);
        // todo:发送好友申请通知
        // 创建好友关系
        // 判断是否已经是好友关系
        ...
        // 是否被拉黑了
        if(blacklistsService.getOne(new LambdaQueryWrapper<>(BlacklistsPO.class).eq(BlacklistsPO::getTargetId,currentId).eq(BlacklistsPO::getOwnerId,friendId))!=null){
    ...
    public PageResult<FriendlistVO> getFriendlist(Integer page, Integer pageSize, FriendshipsPO.Status status) {
        ...
        // 自定义sql语句，得到结果的page
        Page<FriendlistVO> pageInfo = friendShipsMapper.getFriendlist(pageSet,userId,status);
        // 封装成PageResult类型并返回
